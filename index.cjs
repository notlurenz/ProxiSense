// Import http and fetch libraries
const http = require('http');
const fetch = require('node-fetch'); // Ensure you install this package using 'npm install node-fetch'
...
const server = http.createServer(async (request, response) => {
    ...
    if (method === 'GET' && url === '/') {
        ...
        try {
            // Fetch weather data
            const apiUrl = 'https://api.weatherapi.com/v1/current.json?key=99755e46731a4b1b81730018242109&q=14.611581277767675,120.9627894366885';
            ...
            response.write(JSON.stringify({ error: 'Failed to fetch weather data' }));
            response.end();
        }
    } else {
